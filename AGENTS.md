...
| `book/grocery.py` | ingredient lines to shopping entries: amounts, aisles, staples |
| `book/diet.py` | what protein a recipe contains, and what it is built on |
| `book/planner.py` | regions, protein families, the planner payload, the default week |
...
`site/plan.html` picks a week of meals and turns it into one shopping list and a
printable pack. It is generated by `build_plan()` in `site.py` and runs in the browser
from `plan.js`, on data derived at build time by `grocery.py`, `diet.py` and `planner.py`.

**It picks from three of the book's four courses.** `planner.COURSES` names them —
dinners 01-50, breakfasts 51-70, puddings 71-85 — and every record carries the `sec` it
belongs to. The sides, 86-100, are deliberately not pickable: a side is chosen to go
beside a main rather than on its own, and `pairings.py` already answers that question on
the recipe pages. Breakfast and pudding default to nought. The diet ticks govern all
three courses; the pan ticks are read against the dinners alone, because nobody unticks
the wok to rule out a bowl of yoghurt.

**A meal can also be looked up and put in by hand.** The search indexes the title, the
cuisine, the region, the protein, the pan and every ingredient name, so `anchovy` finds
the three dishes carrying one. It deliberately offers matches the ticks above would have
excluded: choosing a dinner by hand is not the filter's business. Anything picked can be
taken out again, and the week in the address bar is the record of what is in it — the
course is a property of the recipe, so it is not carried there twice.

**The protein filter is strict, and that is the point.** `diet.py` tags a recipe with
every kind of flesh it contains, not with the one the dish is named after: a chicken
stir-fry seasoned with fish sauce carries both `chicken` and `fish`, and unticking
either one drops it. Eight of the fifty dinners carry fish only as a seasoning —
anchovy melted into oil, fish sauce, shrimp paste — and a filter that quietly served
one of those to somebody who had unticked Fish is the one failure on this site that
would actually matter. The tags are derived from the ingredient names by keyword, and
`diet.py`'s `__main__` cross-checks every one of them against the `**Vegetarian**`
marker in the recipe files; that check reports zero disagreements today and a change
that breaks it is a bug. It is still a filter on what the recipes say, not an allergen
check, and the page says so.

**The shopping list never drops a line.** `grocery.py` parses all 1,288 ingredient
lines; run `python3 book/grocery.py` for the audit. It prints what it dropped (water,
and one line of reserved fennel fronds) and anything it could not file under an aisle.
An ingredient it cannot read keeps its own words and still lands on the list, because
a list that quietly loses an onion is worse than an untidy one. Amounts only ever add
up within their own family — grams with grams, spoons with spoons — since turning six
tablespoons of oil into 90 ml implies a precision the recipes do not have.

**The week the page ships with is deterministic.** `planner.default_week()` takes no
seed and no date; it walks the dinners in order and takes each one that brings a
protein, a pan and a region the week does not have yet. CI byte-compares the committed
`site/` against a fresh build, so anything random here would turn the repo red on the
next run.

**The PDF is written by hand, in `book/web/pdf.js`.** The site makes no external
requests, so there is no library to load; it emits PDF 1.4 bytes using the fourteen
fonts every reader already has. Two things there are easy to break and silent when you
do: a JavaScript string is not bytes (`new Blob([str])` encodes as UTF-8 and every
offset after the first accented character is then wrong — Preview repairs it quietly,
which is how you ship it), and each cross-reference entry is exactly twenty bytes
including its two-byte line ending. If you touch that file, check the result with
`qpdf --check`, `cupsfilter` and a render, not by opening it in Preview.
...
`make site` also writes `site/assets/recipes.js`, the fifty dinners in full. It is
180 kB and only the planner's recipe pack needs it, so the page loads it on demand
rather than embedding it in every visit.
