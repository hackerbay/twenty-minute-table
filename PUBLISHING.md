...
The ISBNs are printed in the book and submitted to KDP, but a number is only *yours* in the
public record once it is registered against the title at [myidentifiers.com](https://www.myidentifiers.com).
Until then the book has an ISBN that resolves to nothing.

Everything below is settled. Sign in, open each ISBN, and type it.
...
**Subjects.** Three BISAC headings, in order: *Cooking / Methods / Quick & Easy*,
*Cooking / Courses & Dishes / General*, *Family & Relationships / Parenting / General*.
Pick them from Bowker's own subject picker rather than typing codes, so the exact strings are
the ones Bowker recognises. (The three-subject limit is Bowker's; the two-category limit
elsewhere in this document is KDP's, on a different form.)

Four things the form asks for that this repo cannot answer:
...
- **A contributor biography and location** have to be written. Nothing in the repo supplies one.

Confirm before saving that **HackerBay** exists as an imprint under the account. Bowker treats
an imprint as an account-level fact and it cannot be quietly renamed afterwards.
